trigger UpdateProductConfigStatusFromRelease on ProductRelease__c (after update) {
 if(LX_CommonUtilities.ByPassBusinessRule()) return; // [AK 08-Aug-13] : Added Bypass code
    ...
    for (ProductRelease__c newReleases:Trigger.new){
        if (newReleases.status__c != Trigger.oldMap.get(newReleases.Id).status__c) {
            ProductReleaseIDs.add(newReleases.id);
        }
    }
    if (ProductReleaseIDs.size() > 0){
        UpdateProductConfig.UpdateProductConfig(ProductReleaseIDs); 
    }
