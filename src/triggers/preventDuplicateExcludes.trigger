trigger preventDuplicateExcludes on Price_Book_Exclude__c(before insert,before update) 
{
if(LX_CommonUtilities.ByPassBusinessRule()) return; // [AK 08-Aug-13] : Added Bypass code
map<ID,list<Price_Book_Exclude__c>> pbd_ed = new map<ID,list<Price_Book_Exclude__c>>();
map<ID,Price_Book_Exclude__c> ed_map = new map<ID,Price_Book_Exclude__c>();
set<ID> edID = new set<ID>();
list<Price_Book_Exclude__c> ed_lst = new list<Price_Book_Exclude__c>();

list<Price_Book_Exclude__c> temp = new list<Price_Book_Exclude__c>();

for(Price_Book_Exclude__c ed : trigger.new)
{
edID.add(ed.Price_Book_Definition__c);
}


for(Price_Book_Exclude__c ed : [Select id,Product__c,Product_Model__c,Price_Book_Definition__c from Price_Book_Exclude__c ed where ed.Price_Book_Definition__c in :edID])
{
...
for(Price_Book_Exclude__c ed : ed_lst)
{
...
if(pbd_ed.Containskey(ed.Price_Book_Definition__c))
{
for(Price_Book_Exclude__c edd : pbd_ed.get(ed.Price_Book_Definition__c))
{
